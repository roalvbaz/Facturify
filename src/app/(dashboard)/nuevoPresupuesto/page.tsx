...
/**
 * Entrada única para crear documentos:
 *  - Por defecto → nuevo PRESUPUESTO (el flujo principal de facturación).
 *  - ?editar=<id> → editar un presupuesto existente (cambios del cliente).
 *  - ?mode=rectification → factura rectificativa (Serie R) directamente.
 */
export default function NuevaFacturaPage() {
  const [view, setView] = useState<'loading' | 'presupuesto' | 'rectificativa'>('loading');
  ...
  useEffect(() => {
    ...
    setEditId(editar);
    setView(sp.get('mode') === 'rectification' ? 'rectificativa' : 'presupuesto');
  }, []);

  if (view === 'rectificativa') {
    return <NuevaFacturaDirecto />;
  }

  if (view === 'presupuesto') {
    return <EstimateForm mode={editId ? 'edit' : 'new'} estimateId={editId || undefined} />;
  }

  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
      <i className="fas fa-spinner fa-spin" style={{ fontSize: '1.6rem', color: 'var(--text-muted)' }}></i>
    </div>
  );
}

/** Formulario de factura directa (solo serie R, rectificativa). */
function NuevaFacturaDirecto() {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            <div>
              ...
              <input 
                ...
                style={{ backgroundColor: 'var(--bg-color)', opacity: 0.8, cursor: 'not-allowed', height: '42px', fontSize: '0.85rem', color: seriesCode === 'R' ? 'var(--danger)' : undefined, fontWeight: seriesCode === 'R' ? 700 : undefined }} 
              />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '0.3rem' }}>
